...
colnames(A3a)[colnames(A3a) == "L1.Population"] <- "L1_pop"
# Remove all NA entries
A3a <- na.omit(A3a) 
paste("Size of dataset with NA remove:", dim(A3a)[1])
paste("#Island_Endemic: ", sum(A3a$Island.Endemic))
paste("#Island_Endemic to Total Ratio: ", sum(A3a$Island.Endemic) / dim(A3a)[1])
# Convert variable types
A3a$Island.Endemic <- as.numeric(A3a$Island.Endemic)
# Rename predictors
colnames(A3a)[colnames(A3a) == "L1.Population"] <- "L1_pop"
...
save(phylomatrix, file = "output/A3a/phylomatrix.RData") 
save(spmatrix, file = "output/A3a/spmatrix.RData")
...
load("output/A3a/phylomatrix.RData")
load("output/A3a/spmatrix.RData")
...
# res <- gls(model=Phoneme.Inventory.Size~1,data=A3a,correlation=corSymm(mat[lower.tri(mat)],fixed=T),method="ML")
# print(-res$logLik)
...
save(p.res, file = "output/A3a/ml_p_res.RData")
...
ml_fits <- mclapply(
  X = models,
  FUN = function(f) ml_fit(p=p.res$par,formula=f,data=A3a,spmatrix=spmatrix,phylomatrix=phylomatrix),
  mc.cores = 8
)
# Save models
saveRDS(ml_fits, "output/A3a/ml_fits.RDS") 
...
for (i in 1:n){
  ...
  if(dim(ml_summary$tTable)[1] == 1) {
    ...
    ml_MSE <- c(ml_MSE, sum(ml_fits[[i]]$residuals**2)/dim(A3a)[1])
    next
  }
...
# Save output
write.csv(ml_data, file = "output/A3a/ml_data.csv", row.names=FALSE)
